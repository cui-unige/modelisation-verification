...
public extension PTNet {
  ...
  public func markingGraph(from marking: PTNet<Place>.MarkingType) -> MarkingGraph<Place>? {
    // TODO: Write here the implementation of the marking graph generation.
    
    let marking_graph = MarkingGraph<Place>(marking: marking, successors: [:])
    
    var work = [marking_graph]
    var seen = [MarkingGraph<Place>]()

    while work.count != 0 {
      let cur_work = work.popLast()
      seen.append(cur_work!)
      let cur_marking = cur_work!.marking
      for transition in self.transitions {
        if let new_marking = transition.fire(from: cur_marking) {
          if seen.contains(where: { $0.marking == new_marking}) {
            cur_work?.successors[transition] = seen.first(where: { $0.marking == new_marking})
          }
          else if work.contains(where: { $0.marking == new_marking}) {
            cur_work?.successors[transition] = work.first(where: { $0.marking == new_marking})
          }
          else {
            let new_marking_graph = MarkingGraph<Place>(marking: new_marking)
            cur_work!.successors[transition] = new_marking_graph
            work.append(new_marking_graph)
          }
        }
      }
    }

  return marking_graph
...
public extension PTNet {
  ...
  public typealias CoverabilityMarking = [Place: Token]
  ...
  public func coverabilityGraph(from marking: CoverabilityMarking) -> CoverabilityGraph<Place> {
    // Create the initial node of the coverability graph.
    let initialNode = CoverabilityGraph<Place>(marking: marking)
  ...
  func computeSuccessors(
    of currentNode: CoverabilityGraph<Place>,
    seen          : inout [CoverabilityGraph<Place>],
    predecessors  : [CoverabilityGraph<Place>])
  {
    for transition in self.transitions {
      
      //----------------------------------------------------
      
      // TODO: Compute successors (Think to use functions below with PTTransition)
      // Your code here
      ...
      // Uncomment lines below with /* */ when you'll create successor
...
public extension PTTransition {
  ...
  public func isFireable(from marking: CoverabilityMarking) -> Bool {
    for arc in self.preconditions {
      ...
      if marking[arc.place]! < .some(arc.label) {
        return false
      }
    }
  ...
  public func fire(from marking: CoverabilityMarking) -> CoverabilityMarking? {
    ...
    for arc in self.preconditions {
      ...
      case .some(let tokens):
        ...
        result[arc.place] = .some(tokens - arc.label)
      case .omega:
